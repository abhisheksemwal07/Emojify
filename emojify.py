...
def draw_arc(img, x, y, rotation, radius):
    ...
    cv2.ellipse(img, center, axes, angle, startAngle, endAngle, (255, 255, 255), int(radius / 2))
        ...
        for j in range(len(faces)):
            for i in range(len(faces[j])):
                ...
                cv2.ellipse(img, (face_pos[0], face_pos[1] - 30), (int(faceLength / 1.5), int(faceLength / 1.5) + 10),
                            find_rotation(faces[j][face['faceUp']], faces[j][face['faceDown']]), 0, 360, (52, 225, 255),
                            -1)

                Right_eye_length, R_eye_pos = findDistance(faces[j][rightEye['eyeUp']], faces[j][rightEye['eyeDown']],
                                                           img)

                cv2.circle(img, (R_eye_pos[0], R_eye_pos[1] - 30), int(Right_eye_length / 1.3), (65, 71, 100), -1)

                draw_arc(img, R_eye_pos[0] - 3, R_eye_pos[1] - 33,
                         find_rotation(faces[j][rightEye['eyeRight']], faces[j][rightEye['eyeLeft']]),
                ...
                draw_arc(img, L_eye_pos[0] - 3, L_eye_pos[1] - 33,
